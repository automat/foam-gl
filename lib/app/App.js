...
function App(canvas,resources) {
    ...
    if(this.__loop){
        var performance;
        var start, now, previous, elapsed, frameTime, deltaTime;
        var numFixedTicks, accumulator;
        var fixedStep, targetFPS, stepDuration;
        var tick;
        ...
        targetFPS    = 1000 / this.__targetFPS;

        tick = this.__hasFixedUpdate ?
            function(timestamp){
                timestamp = timestamp || 0;

                Time.__now     = timestamp;
                Time.__elapsed = Math.max(0,timestamp - start);

                Time.__secondsElapsed = Time.__elapsed * 0.001;
                Time.__frameTime      = frameTime = timestamp - previous;
                Time.__delta          = deltaTime = frameTime * 0.001;
                ...
                self.update(deltaTime,accumulator/stepDuration);
