...
function Arcball(camera,bounds){
    ...
    this._center = bounds ? new Vec2(bounds.getWidth() * 0.5, bounds.getHeight() * 0.5) : app.getWindowSize().scale(0.5);
    this._updateRadius();

    var tempVec2_0 = new Vec2(),
        tempVec2_1 = new Vec2();

    app.addEventListener(WindowEvent.RESIZE,function(){
        if(self._bounds){
            return;
        }
    ...
    mouse.addEventListener(MouseEvent.MOUSE_DOWN,function(e){
        ...
        var windowHeight = app.getWindowHeight(),
            bounds       = self._bounds;
