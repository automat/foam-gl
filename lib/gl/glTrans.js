...
glTrans.translate3f = function (x, y, z) {
    //matrixModelView.mult(Matrix44.fromTranslation(x,y,z,matrixTemp0.identity()));
...
glTrans.scale3f = function (x, y, z) {
    //matrixModelView.mult(Matrix44.fromScale(x,y,z,matrixTemp0.identity()));
...
glTrans.rotate = function (v) {
    //matrixModelView.mult(Matrix44.fromRotation(v.x, v.y, v.z, matrixTemp0.identity()));
...
glTrans.rotate3f = function (x, y, z) {
    //matrixModelView.mult(Matrix44.fromRotation(x,y,z,matrixTemp0.identity()));
    matrixModelView.rotatef(x,y,z);
    matrixNormalDirty = true;
};

glTrans.rotateAxis = function (angle, v) {
    //matrixModelView.mult(Matrix44.fromRotationOnAxis(angle, v.x, v.y, v.z, matrixTemp0.identity()));
    matrixModelView.rotateFromAxisf(angle,v);
    matrixNormalDirty = true;
};

glTrans.rotateAxis3f = function (angle, x, y, z) {
    //matrixModelView.mult(Matrix44.fromRotationOnAxis(angle, x, y, z, matrixTemp0.identity()));
...
glTrans.rotateQuat = function(q){
    matrixModelView.mult(q.toMatrix44(matrixTemp0));
    matrixNormalDirty = true;
};
