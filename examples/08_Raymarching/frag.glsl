...
vec2 obj1(in vec3 p){
    //return vec2(length(max(abs(p)-vec3(1,1,1),0.0))-0.3,1);
    return vec2(length(p)-1.0,1);
}
...
void main(void){
    vec2 vPos=-1.0+2.0*gl_FragCoord.xy/uScreenSize.xy;
    
    //Camera animation
    vec3 vuv=vec3(0,1,0);//Change camere up vector here
    vec3 vrp=vec3(0,0,0); //Change camere view here
    vec3 prp=vec3(-sin(uTime)*20.0,4,cos(uTime)*8.0); //Change camera path position here
    
    //Camera setup
    vec3 vpn=normalize(vrp-prp); // w
    vec3 u=normalize(cross(vuv,vpn)); // u
    vec3 v=cross(vpn,u);
    ...
    vec3 scrCoord=vcv+vPos.x*u*uScreenRatio+vPos.y*v;
