...
(function () {
    ...
    // fontSize - TextureFont size, in pixels (default: 72).
    ...
    // A TextureFont represents a loaded OpenType font file.
    ...
    // fontSize - TextureFont size in pixels. We scale the glyph units by `1 / unitsPerEm * fontSize`. (default: 72)
    ...
    function parseHeadTable(data, start) {
        ...
        checkArgument(head.magicNumber === 0x5F0F3CF5, 'TextureFont header has wrong magic number.');
    ...
    function loadFromUrl(url, callback) {
        ...
        request.onload = function () {
            if (request.status !== 200) {
                return callback('TextureFont could not be loaded: ' + request.statusText);
            }
    ...
    // Parse the OpenType file data (as an ArrayBuffer) and return a TextureFont object.
    // If the file could not be parsed (most likely because it contains Postscript outlines)
    // we return an empty TextureFont object with the `supported` flag set to `false`.
    ...
    // the `font` is a TextureFont object.
    //
    ...
    opentype.load = function (url, callback) {
        ...
        loader(url, function (err, arrayBuffer) {
            ...
            if (!font.supported) {
                return callback('TextureFont is not supported (is this a Postscript font?)');
            }
